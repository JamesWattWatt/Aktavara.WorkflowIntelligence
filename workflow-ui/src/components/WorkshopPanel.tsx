...
import type { WorkflowCandidateResult, HelpGuideSection, GuideSuggestion, GuideMapping } from '../types/api';
...
export const WorkshopPanel = ({ candidate, workflowId, detectedUser: _detectedUser, onOpenHelp }: WorkshopPanelProps) => {
  ...
  const [currentMapping, setCurrentMapping] = useState<GuideMapping | null>(null);
  ...
  const loadGuideMappings = async (wId: string) => {
    try {
      const result = await apiClient.getWorkflowGuideMappings(wId);
      if (candidate?.currentStateName) {
        const normalizedStateName = normalizeStepId(candidate.currentStateName);
        const mapping = (result.mappings || []).find(m => m.stepId === normalizedStateName);
        setCurrentMapping(mapping || null);
      }
    } catch (error) {
      console.error('Error loading guide mappings:', error);
    }
  };
  ...
  const approveSuggestion = async () => {
    ...
    try {
      ...
      // Reload mappings to update current mapping status
      if (candidate.workflowId) {
        await loadGuideMappings(candidate.workflowId);
      }
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="border-t border-gray-200 dark:border-gray-700 pt-4">
        ...
        {/* Show guide content if approved mapping exists */}
        {currentMapping && !currentMapping.isAutoGenerated && helpGuide ? (
          <div className="space-y-2">
            <div className="flex items-center gap-2">
        ...
        ) : currentMapping && currentMapping.isAutoGenerated ? (
          /* Show suggested mapping card */
          <div className="space-y-3">
            {showSuggestionUI && guideSuggestion ? (
              <div className="p-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg">
                <div className="flex items-center gap-2 mb-2">
                  <div className="w-3 h-3 rounded-full bg-amber-500"></div>
                  <span className="text-xs font-medium text-amber-700 dark:text-amber-300">💡 Suggested</span>
                </div>
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className="flex-1 px-3 py-2 border border-amber-300 dark:border-amber-700 text-amber-700 dark:text-amber-400 rounded text-sm font-medium hover:bg-amber-50 dark:hover:bg-amber-900/30 transition-colors"
                  >
                    ✗ Dismiss
                  </button>
                </div>
              ...
              <>
                <div className="flex items-center gap-2 p-3 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg">
                  ...
                  <div className="flex-1">
                    <span className="text-xs font-medium text-amber-700 dark:text-amber-300">💡 Suggested Guide</span>
                    <p className="text-xs text-amber-600 dark:text-amber-400">Auto-suggested, awaiting review</p>
                  </div>
                </div>
                <button
                ...
                >
                  {suggestingGuide ? 'Loading...' : 'View Suggestion'}
                </button>
              </>
            )}
          </div>
        ) : candidate.nextStepHint ? (
          /* Show "Suggest guide" button when no mapping */
          <div className="space-y-3">
            <button
              onClick={suggestGuideMapping}
              disabled={suggestingGuide}
