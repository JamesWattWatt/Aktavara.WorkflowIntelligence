...
function GuidesTab({ definition }: GuidesTabProps) {
  ...
  const [approvingState, setApprovingState] = useState<string | null>(null);
  const [dismissedStates, setDismissedStates] = useState<Set<string>>(new Set());
  ...
  async function loadMappings() {
    try {
      ...
      const result = await apiClient.getWorkflowGuideMappings(definition!.workflowId);
      const mappingsByState = (result.mappings || []).reduce((acc: any, m: any) => {
        acc[m.stepId] = m;
        return acc;
      }, {});
      setMappings(mappingsByState);
    } catch (err) {
      console.error('Error loading guide mappings:', err);
    } finally {
      setLoadingMappings(false);
    }
  }

  async function suggestGuideForState(state: any) {
    try {
      const suggestion = await apiClient.suggestGuideMapping(
        ...
        state.stateId,
        state.name,
        definition?.activitySignature?.map(r => r.description) || [],
        []
      );
      setSuggestions(prev => ({
        ...prev,
        [state.stateId]: suggestion
      }));
    } catch (err) {
      console.error('Error suggesting guide:', err);
    }
  }

  async function approveMapping(stateId: string) {
    const suggestion = suggestions[stateId];
    if (!suggestion) return;

    try {
      setApprovingState(stateId);
      await apiClient.saveGuideMapping(
        definition!.workflowId,
        stateId,
        suggestion.suggestedGuideFile,
        suggestion.suggestedSectionId || undefined
      );
  ...
  function dismissSuggestion(stateId: string) {
    setDismissedStates(prev => new Set(prev).add(stateId));
    setSuggestions(prev => {
      const next = { ...prev };
      delete next[stateId];
      return next;
  ...
  return (
    <div className="space-y-4 p-4">
      {loadingMappings && <p className="text-sm text-gray-500">Loading mappings...</p>}

      {(definition.states || []).map((state) => {
        const mapping = mappings[state.stateId];
        const suggestion = suggestions[state.stateId];
        const isDismissed = dismissedStates.has(state.stateId);

        const status = mapping?.isAutoGenerated === true ? 'suggested' : mapping ? 'approved' : 'none';

        return (
          <div key={state.stateId} className="border border-gray-200 dark:border-gray-700 rounded-lg p-3">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-shrink-0 pt-1">
                ...
                {status === 'suggested' && (
                  <div className="w-3 h-3 rounded-full bg-amber-500" title="Suggested"></div>
                )}
                {status === 'none' && (
                  <div className="w-3 h-3 rounded-full bg-gray-300 dark:bg-gray-600" title="No mapping"></div>
                )}
              </div>
              ...
              <div className="flex-1 min-w-0">
                ...
                {status === 'approved' && (
                  <div className="mt-2 p-2 bg-green-50 dark:bg-green-900/20 rounded border border-green-200 dark:border-green-800">
                    ...
                    <p className="text-xs text-gray-700 dark:text-gray-300">
                      <span className="font-medium">Guide:</span> {mapping.guideFile}
                    </p>
                    {mapping.sectionId && (
                      <p className="text-xs text-gray-700 dark:text-gray-300">
                        <span className="font-medium">Section:</span> {mapping.sectionId}
                      </p>
                    )}
                  </div>
                )}

                {status === 'suggested' && !isDismissed && (
                  <div className="mt-2 p-3 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded">
                    <p className="text-xs font-medium text-amber-700 dark:text-amber-300 mb-2">💡 Suggested</p>
                    {suggestion ? (
                      <>
                        <p className="text-xs text-gray-700 dark:text-gray-300 mb-1">
                          <span className="font-medium">Guide:</span> {suggestion.suggestedGuideFile}
                        </p>
                        <p className="text-xs text-gray-700 dark:text-gray-300 mb-1">
                          <span className="font-medium">Section:</span> {suggestion.suggestedSectionHeading}
                        </p>
                        <p className="text-xs text-gray-700 dark:text-gray-300 mb-2">
                          <span className="font-medium">Reason:</span> {suggestion.confidenceReason}
                        </p>
                        <div className="flex gap-2">
                          <button
                            onClick={() => approveMapping(state.stateId)}
                            disabled={approvingState === state.stateId}
                            className="flex-1 px-2 py-1 bg-green-600 hover:bg-green-700 disabled:opacity-50 text-white rounded text-xs font-medium"
                          >
                            ✓ Approve
                          </button>
                          <button
                            onClick={() => dismissSuggestion(state.stateId)}
                            className="flex-1 px-2 py-1 border border-amber-300 dark:border-amber-700 text-amber-700 dark:text-amber-400 rounded text-xs font-medium hover:bg-amber-50 dark:hover:bg-amber-900/30"
                          >
                            ✗ Dismiss
                          </button>
                        </div>
                      ...
                      <button
                        onClick={() => suggestGuideForState(state)}
                ...
                {isDismissed && (
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">Suggestion dismissed</p>
                )}
              </div>
            </div>
          </div>
