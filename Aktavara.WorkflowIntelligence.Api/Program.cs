...
// Get guide mappings for a workflow endpoint
app.MapGet("/api/workflows/{id}/guide-mappings", GetWorkflowGuideMappings)
.WithName("GetWorkflowGuideMappings")
.WithOpenApi()
.WithDescription("Get all guide mappings for a workflow")
.Produces<object>(200)
.Produces(404);
    ...
    ILogger<Program> logger)
{
    ...
    try
    {
        ...
        var response = new AnalyzeResponse
        {
            ...
            DetectedUser = detectedUser ?? string.Empty,
    ...
    ILogger<Program> logger)
{
    try
    {
        ...
        var response = new AnalyzeResponse
        {
            ...
            RecommendedNextStep = packet.RecommendedNextStep,
            ContextNarrative = packet.ContextNarrative ?? string.Empty,
            DetectedUser = userName ?? string.Empty,
            ActiveEntities = packet.ActiveEntities ?? new(),
            WorkflowHints = packet.WorkflowHints ?? new(),
            SemanticMatches = packet.SemanticMatches ?? new(),
            Ambiguity = packet.Ambiguity,
...
IResult GetWorkflowGuideMappings(string id, IConfiguration config)
{
    try
    {
        var helpGuidesPath = config.GetValue<string>("WorkflowIntelligence:HelpGuidesPath") ?? "help-guides/";
        var mappingsPath = Path.Combine(helpGuidesPath, "workflow-guide-mapping.json");

        if (!File.Exists(mappingsPath))
            return Results.Ok(new { workflowId = id, mappings = new List<object>() });
        ...
        var workflowMappings = new List<Dictionary<string, object?>>();
        foreach (var mapping in mappingsArray.EnumerateArray())
        {
            if (mapping.TryGetProperty("workflowId", out var wf) &&
                wf.GetString() == id)
            {
                var entry = new Dictionary<string, object?>
                {
                    { "stepId", mapping.TryGetProperty("stepId", out var st) ? st.GetString() : null },
                    { "guideFile", mapping.TryGetProperty("guideFile", out var gf) ? gf.GetString() : null },
                    { "sectionId", mapping.TryGetProperty("sectionId", out var si) && si.ValueKind != System.Text.Json.JsonValueKind.Null ? si.GetString() : null },
                    { "isAutoGenerated", mapping.TryGetProperty("isAutoGenerated", out var ag) ? ag.GetBoolean() : false }
                };
                workflowMappings.Add(entry);
            }
        }

        return Results.Ok(new { workflowId = id, mappings = workflowMappings });
    }
    catch
    {
        return Results.BadRequest(new { error = "Error fetching guide mappings" });
    }
}
