    ...
    const tools = [
        {
            ...
            description: 'Analyze recent Aktavara user activity from a log snippet to detect workflow patterns and provide guidance',
            inputSchema: {
                type: 'object',
                properties: {
                    logContent: {
                    ...
                    userQuestion: {
                        type: 'string',
                        description: 'Optional: what the user says they are trying to do'
                    }
                },
        ...
        },
        {
            ...
            description: 'Detect what workflow task the user is currently performing based on their activity and optional description',
            inputSchema: {
                type: 'object',
                properties: {
                    logContent: {
        ...
        },
        {
            ...
            description: 'Get the full definition of a named workflow including its steps, states, and rules',
            inputSchema: {
                type: 'object',
                properties: {
                    workflowId: {
        ...
        {
            name: 'get_help_guide',
            description: 'Get help guide content for a specific Aktavara workspace or feature',
            inputSchema: {
                type: 'object',
                properties: {
                    helpGuideId: {
                    ...
                    sectionId: {
                        type: 'string',
                        description: 'Optional: specific section ID within the guide'
                    }
                },
        ...
        },
        {
            ...
            description: 'Get a complete explanation of what the user should do next, grounded in their current activity and the relevant help documentation',
            inputSchema: {
                type: 'object',
                properties: {
                    logContent: {
    ...
    try {
        ...
        switch (request.params.name) {
            case 'analyze_activity':
                result = await analyzeActivityTool.analyzeActivity(analyzeActivityTool.analyzeActivitySchema.parse(request.params.arguments));
                break;
            case 'detect_current_task':
                result = await detectTaskTool.detectTask(detectTaskTool.detectTaskSchema.parse(request.params.arguments));
                break;
            case 'get_workflow_definition':
                result = await getWorkflowTool.getWorkflow(getWorkflowTool.getWorkflowSchema.parse(request.params.arguments));
                break;
            case 'get_help_guide':
                result = await getHelpGuideTool.getHelpGuide(getHelpGuideTool.getHelpGuideSchema.parse(request.params.arguments));
                break;
            case 'explain_next_step':
                result = await explainNextStepTool.explainNextStep(explainNextStepTool.explainNextStepSchema.parse(request.params.arguments));
                break;
            default:
                return {
                    content: [
...
server.setRequestHandler('resources/read', async (request) => {
    const workflowContent = await workflowResources.getWorkflowResourceContent(request.params.uri);
    if (workflowContent) {
        return workflowContent;
    }
    const helpGuideContent = await helpGuideResources.getHelpGuideResourceContent(request.params.uri);
    if (helpGuideContent) {
