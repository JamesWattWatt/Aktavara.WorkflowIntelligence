...
export async function getHelpGuide(input: GetHelpGuideInput): Promise<string> {
  ...
  if (guide.sections.length > 0) {
    ...
    topLevelSections.forEach(section => {
      ...
      // Add child sections
      const childSections = guide.sections.filter(
        s => s.parentSectionId === section.sectionId
      );
