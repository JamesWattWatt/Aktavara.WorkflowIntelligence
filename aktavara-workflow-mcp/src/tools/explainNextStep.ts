...
export async function explainNextStep(input: ExplainNextStepInput): Promise<string> {
  ...
  if (response.workflowCandidates.length > 0) {
    ...
    lines.push('');
    lines.push(
      'The activity does not clearly match any known workflow pattern.'
    );
    lines.push('');
  }
  ...
  if (response.ambiguity?.isAmbiguous && response.ambiguity.clarificationQuestion) {
    ...
    lines.push('');
    lines.push(
      `**Question:** ${response.ambiguity.clarificationQuestion}`
    );
    lines.push('');
  }
