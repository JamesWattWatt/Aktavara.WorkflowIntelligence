...
export async function analyzeActivity(input: AnalyzeActivityInput): Promise<string> {
  ...
  if (response.activeEntities.length > 0) {
    ...
    response.activeEntities.forEach(entity => {
      lines.push(
        `  - ${entity.recordKind}: ${entity.name} (ID: ${entity.recordId})`
      );
    });
  }
  ...
  if (response.workflowCandidates.length > 0) {
    ...
    lines.push(
      `**Confidence:** ${(topMatch.confidenceScore * 100).toFixed(0)}% (${topMatch.confidenceLevel})`
    );
  ...
  if (response.semanticMatches.length > 0) {
    ...
    response.semanticMatches.slice(0, 3).forEach(match => {
      lines.push(
        `**${match.workflowName}** (${(match.score * 100).toFixed(0)}%)`
      );
  ...
  if (response.ambiguity?.isAmbiguous) {
    ...
    lines.push(
      `**Recommended Action:** ${response.ambiguity.recommendedAction}`
    );

    if (response.ambiguity.clarificationQuestion) {
      lines.push(
        `**Clarification Needed:** ${response.ambiguity.clarificationQuestion}`
      );
    }
