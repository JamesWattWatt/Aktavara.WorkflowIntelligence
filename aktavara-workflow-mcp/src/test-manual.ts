...
async function runTests() {
  ...
  try {
    ...
    const analysisNoQ = await api.analyzeText(
      testLogContent,
      'testuser',
      30
    );
    console.log(
      `   ✓ Current state: ${analysisNoQ.currentState}`
    );
    console.log(
      `   ✓ Top match: ${analysisNoQ.workflowCandidates[0]?.workflowName || 'None'}`
    );
    console.log(
      `   ✓ Guidance level: ${analysisNoQ.guidanceLevel}\n`
    );

    // Test 5: Analyze activity with question
    console.log('5️⃣  Analyzing activity (with question)...');
    const analysisWithQ = await api.analyzeText(
      testLogContent,
      'testuser',
      30,
      'I want to update a node property'
    );
    console.log(`   ✓ Semantic matches: ${analysisWithQ.semanticMatches.length}`);
    if (analysisWithQ.ambiguity) {
      console.log(
        `   ✓ Ambiguity detected: ${analysisWithQ.ambiguity.isAmbiguous}`
      );
    }
    console.log('');
    ...
    const sections = await api.getHelpGuideSection(
      'update-node-in-path',
      'editing'
    );
